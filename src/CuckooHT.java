public class CuckooHT extends HashTable {
    ...
    private long[] tmp; // set of keys already inserted
	...
	long i = hashFuncA.hash(k); 	// pos in tables
	long initInsertPos = -1; 	// init the initial pos we start with to avoid a cycle
	//	while (nRehashes < maxRehashes) {

	//	    for (int swaps = 0; swaps < maxSwaps; swaps++) {
	while (i != initInsertPos) {
     ...
     */
    public void rehash(int nRehashes, long k) {
	...
	if (nRehashes == 0) {
	    ...
	    // Copy first table to temp array 
	    tmp = new long[2 * (int) m];
	    for (int i = 0; i < m; i++) { // init array
		tmp[i] = keysA[i]; // copy
		keysA[i] = -1; // erase
	    }
